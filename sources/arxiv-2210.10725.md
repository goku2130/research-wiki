...
**Key Formulas**
The Skip Logit output is mathematically defined as $\text{logit} = x_{0} + \sum_{i=1}^{L-1} F(x_{i}, W_{i}^{L-1})$, where $x_i$ denotes the $i$-th layer output and $L$ is the total skip paths. This architecture yields linear variance growth: $\text{Var}(f(x)) = L \times \text{Var}(x)$. The MTN operation is formulated as $x_{norm} = W \odot \text{Tanh}(s(x) \odot x)$, where $W$ is a learnable scale parameter and $s(x)$ is the meta variance learner. To reduce computational overhead, the variance learner operates on a stop-gradient input $x'$: $s(x) = \text{leakyrelu}(w \times x')$. The Tanh activation bounds variance via $\text{Var}(\tanh(x)) \leq 1$, resulting in a stabilized final variance of $\text{Var}(f(x)) \leq \text{Var}(x) + L - 1$. Theoretically, the optimization landscape satisfies $\|\nabla f(A)\|_F^2 \geq 4 \sum_{i=1}^{l} (1 - \gamma_i)^2 \delta_{\min}(\Sigma) \|f(A) - C_{opt}\|_F^2$, proving that under specific spectral constraints, all critical points are global minima.
