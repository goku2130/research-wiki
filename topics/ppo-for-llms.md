...
- arxiv:2502.13177
open_questions:
- What is the optimal scaling law for KL coefficient $\beta$ with model size and reward
  magnitude in LLM PPO? No consensus exists across [source:arxiv:2204.05862] ($\beta=0.001$
  for 52B), [source:arxiv:1909.08593] (target KL 0.1–1.0 nats), and [source:arxiv:2307.04964]
  (unspecified).
- Does per-token advantage estimation (via GAE with $\lambda<1$) materially improve
  alignment quality over sequence-level advantage in the bandit limit, or does it
  only add variance? [source:arxiv:2307.04964] vs [source:arxiv:2204.05862] disagree
  in practice.
- Can the RPG framework's [source:arxiv:2505.17508] importance-weighting correction
  for GRPO be validated at scale on frontier models, and does it resolve the reported
  instability of GRPO vs PPO?
- How should the RPG-Style Clip parameters $(\epsilon_1, \epsilon_2, c)$ be scheduled
  during training? [source:arxiv:2505.17508] identifies this as an open direction.
---
