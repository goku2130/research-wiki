...
open_questions:
- Can P3O's pairwise policy gradient be scaled to LLM-sized models with the same stability
  gains observed in the blog post experiments?
- Does DPO's implicit reward modeling regression (N+) persist at larger scales, or
  is it an artifact of the 1B–6.9B range and specific implementation details (EOS-only
  vs all-token loss, $\beta$)?
- What is the minimal "PPO recipe" (advantage normalization, large batch, ref-model
  EMA) needed to reliably beat DPO on open-source models, and does it transfer across
  base model families?
- How much of the reported PPO vs DPO gap on code tasks (CodeContest, APPS) is attributable
  to using ground-truth rewards for PPO vs learned RM for DPO, vs algorithmic differences?
---
...
- **Sample efficiency**: <1% of agent interactions required human feedback; ~700 human queries sufficed for near-oracle performance on simulated robotics; ~1,400 synthetic labels achieved oracle-beating performance; ~5,500 human queries for Atari.
...
**Disagreement on SFT scope.** The N+ study initializes the RM from the SFT model with a randomly initialized linear head [source:arxiv:2403.17031], while the HuggingFace overview notes the reference model for KL regularization is typically the original pre-trained distribution [source:huggingface:navigating-the-rlhf-landscape-from-polic]. No source reports using a *different* model for SFT vs. reference, but the N+ ablation of dropout disabling is a practical detail absent from the original paper.

## Reward Model (RM) Training

**Preference data and loss.** Human labelers rank $K$ model outputs (TL;DR: pairwise comparisons) producing $\binom{K}{2}$ comparisons per prompt [source:arxiv:2203.02155]. The RM $r_\phi(x,y)$ outputs a scalar and is trained with a pairwise ranking loss:
...
The **SFT→RM→PPO pipeline remains the foundational reference** for industrial RLHF (InstructGPT, ChatGPT, GPT-4, Claude, Llama 2/3) but **pure PPO is fading in open research** in favor of **offline preference optimization** and **pairwise on-policy methods** that avoid the RM–RL mismatch and reduce infrastructure burden [source:bair:rethinking-the-role-of-ppo-in-rlhf][source:arxiv:2403.17031]. The N+ reproduction (2024) is valuable precisely because **faithful open-source PPO implementations were scarce** [source:arxiv:2403.17031]. The trajectory is toward **simpler, offline, or reward-translation-invariant algorithms** [source:bair:rethinking-the-role-of-ppo-in-rlhf]. **Hedge:** No source claims PPO is abandoned; it remains the production workhorse where compute and engineering capacity exist, but the *research frontier* has moved to alternatives that mitigate its instability and cost.
...
- **Parameter-efficient RLHF** (LoRA + quantization + packing) makes the full pipeline feasible on more accessible GPU clusters (StackLLaMA: 7B on 3×8 A100-80GB) [source:huggingface:stackllama-a-hands-on-guide-to-rlhf-with].
