...
- arxiv:2606.09073
open_questions:
- Do the Gao et al. (2022) scaling laws hold when the gold standard is human preference
  rather than a fixed 6B RM?
- How do the scaling laws change under iterative RM updates (as in production RLHF
  loops) versus the static proxy/gold setup?
- Can the $R(d)=d(\alpha-\beta\log d)$ form be derived theoretically from the rank-deficiency/OOD
  bootstrapping mechanism identified in the DAA work?
- What is the minimal ensemble size for the Gaussian-truncated KL-DRO estimator to
  outperform the log-MGF estimator in practice?
---
...
3. **Initial Policy**: Models of 1.2B and 6B parameters fine-tuned via SFT.
...
The quadratic form implies a sharp, predictable turnover: beyond $d^*$, gold reward declines quadratically with further optimization [source:arxiv:2210.10760].
...
The logarithmic penalty term $\beta_{\mathrm{RL}} \log d$ yields a slower, more gradual turnover than BoN’s quadratic. RL is significantly less KL-efficient: it consumes far more KL distance to achieve the same level of optimization or overoptimization [source:arxiv:2210.10760]. However, the *relationship* between proxy and gold scores (i.e., the gap) remains similar across both methods, suggesting the overoptimization pathology is rooted in the proxy RM itself, not the optimizer [source:arxiv:2210.10760].
...
In PPO, adding a KL penalty term $\lambda D_{\mathrm{KL}}(\pi \parallel \pi_{\mathrm{init}})$ to the objective does *not* improve the gold-reward-vs-KL frontier; it acts effectively as early stopping, tracing the same curve as unpenalized PPO stopped at different steps [source:arxiv:2210.10760].
...
**Article's analysis:** Gao et al. attribute over-optimization to the *proxy RM's approximation error* relative to a gold RM. The DAA work [source:arxiv:2406.02900] shows the *same functional form* $R(d)=d(\alpha-\beta\log d)$ emerges *without any explicit reward model*, implying the pathology is more fundamental: it arises from **optimizing an under-constrained objective on finite preference data**, whether that objective is an explicit RM or a direct preference loss. The "proxy" in DAAs is the *implicit reward* defined by the policy's log-ratio against the reference, and its over-optimization stems from the same rank deficiency / OOD extrapolation mechanism.
