...
- arxiv:2607.08065
open_questions:
- Can architectural interventions (e.g., attention modifications, positional encoding
  changes) fundamentally reduce position bias rather than prompt-level workarounds?
- How to construct a judge evaluation benchmark that is both correctness-focused and
  domain-general, avoiding the "pipeline bias" where models judge their own generations?
- What is the minimal human annotation budget needed to calibrate conformal prediction
  intervals for a new domain, and how does calibration transfer across domains?
- Can we develop a "bias profile" fingerprint for each judge model that predicts its
  behavior on unseen tasks, enabling judge selection per task?
---
...
Model capacity strongly modulates bias susceptibility. Smaller models (e.g., Qwen2.5-Coder-3B) exhibit stronger, less consistent position biases and rely on shallow heuristics [source:mbrenndoerfer:position-bias-in-llm-judges-measurement-][source:arxiv:2604.16790]. Capable judges (GPT-4, Claude-3.5-Sonnet, Llama-3.7-30B) show high repetition stability (RS > 0.95), confirming bias is systematic, not stochastic [source:aclanthology:a-systematic-study-of-position-bias-in-l].
...
- Few-shot prompting raises GPT-4 consistency to 77.5% [source:arxiv:2306.05685].
- **Task and benchmark dependence**: Claude-3.5-Sonnet is nearly fair on MT-Bench (PF = 0.01) but shows strong recency bias on DevBench (PF = 0.22) [source:aclanthology:a-systematic-study-of-position-bias-in-l]. **Disagreement**: The ACL study shows position bias is highly volatile across tasks and benchmarks, contrary to the implication of a consistent judge property in earlier work.
- **List-wise scaling**: When evaluating 3–4 candidates, most models' RR drops below 0.5 [source:arxiv:2410.02736]. Pairwise studies underestimate real-world bias in multi-candidate settings.
...
- Zheng et al.: GPT-4 was significantly more resilient to a "repetitive list" verbosity attack than GPT-3.5 and Claude-v1, which both had failure rates of 91.3% [source:arxiv:2306.05685].
...
- **Self-enhancement**: Models rate their own outputs higher. Wang et al. find "significant bias" across all six tested models [source:arxiv:2410.02736].
...
|----------|-----------|-------------------|
| **Conservative swap** | Run both orders; declare win only if same winner both times | Reduces position bias but doubles cost [source:arxiv:2306.05685] |
...
- **Chatbot Arena**: ~30k crowdsourced battles [source:arxiv:2411.15594].
- **CALM / JudgeBench**: 4,356 samples for bias quantification; 350 questions (154 Knowledge, 98 Reasoning, 56 Math, 42 Coding) for correctness-focused evaluation [source:arxiv:2411.15594][source:arxiv:2410.12784].
